...
Ref.URL --> https://www.freecodecamp.org/news/binary-search-in-python-with-examples/
...
A search algorithm will help us retrieve data from any data structure:
    ...
    LINEAR SEARCH:
        ...
        Searches one element after another until it finds the correct key and value

    DIJKSTRA SEARCH:
        Used for advanced searches
            t maps the shortest distance between 2 nodes (Often, this nodes are route networks)
    ...
    SORTING ALGORITHMS
        Accepts unsorted lists as an input
...
BINARY SEARCH IN DEPTH:
    ...
    It will pick the middle element and compares it with the item we are looking for
    If the comparison is correct then the process ends
    If not, the middle element is either greater or smaller than the item you are looking for
...
## We create a sorted list, otherwise we need to sort the list we have available ##
